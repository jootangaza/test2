...
class lstm_model():
    ...
    def __init__(self, FLAGS, initializer= tf.initializers.GlorotUniform()):
       # tf.contrib.layers.xavier_initializer()):
        ...
        # input data
        self.input_x = tf.compat.v1.placeholder("int64", [None, self.n])
        self.s_len = tf.compat.v1.placeholder("int64", [None])
        self.IND = tf.compat.v1.placeholder(tf.float32, shape=[None, self.s_cnum])
    ...
    def instantiate_weights(self):
        ...
        with tf.name_scope("self_attention_w_s1"):
            # shape(W_s1) = d_a * 2u
            self.W_s1 = tf.compat.v1.get_variable('W_s1', shape=[self.d_a, 2*self.u],
                initializer=self.initializer)
            # shape(W_s2) = r * d_a
            self.W_s2 = tf.compat.v1.get_variable('W_s2', shape=[self.r, self.d_a],
                initializer=self.initializer)
    ...
    def inference(self):
        ...
        cell_fw = tf.compat.v1.nn.rnn_cell.LSTMCell(self.u)
        cell_bw = tf.compat.v1.nn.rnn_cell.LSTMCell(self.u)

        H, _ = tf.compat.v1.nn.bidirectional_dynamic_rnn(
            cell_fw,
            cell_bw,
            input_embed,
            self.s_len,
    ...
    def loss(self):
        ...
        self_atten_loss = tf.square(tf.norm(self_atten_mul - np.identity(att_matrix_size)))
        return 1000 * loss_val + self.alpha * tf.reduce_mean(self_atten_loss)

    def train(self):
        train_op = tf.compat.v1.train.AdamOptimizer(self.learning_rate).minimize(self.loss_val)
        return train_op
